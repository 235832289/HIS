...
namespace com.digitalwave.iCare.middletier.HIS.Reports
{
    ...
    [ObjectPooling(true)]
    public class clsRegisterSvcAuto : com.digitalwave.iCare.middletier.clsMiddleTierBase
    {
        public clsRegisterSvcAuto()
        {
            //
            // TODO: 在此处添加构造函数逻辑
            //
...
//            ORDER BY a.invoiceno_vchr, a.seqid_chr";
        ...
        /// <returns></returns>
        [AutoComplete]
        public long GetCheckOutData(System.Security.Principal.IPrincipal p_objPrincipal, string OPREMPID, string strDate, string strRptId, out DataTable dtCheckOut)
        {
        ...
        /// <returns></returns>
        [AutoComplete]
        public long GetCheckOutData(System.Security.Principal.IPrincipal p_objPrincipal,int intMode, string OPREMPID, string strStartDate,string strEndDate, string strRptId, out DataTable dtCheckOut)
        {
            if (strRptId == null || strRptId == "")
            {
                throw new Exception("报表的Id号为空，请从功能菜单传入报表Id号。");
  ...
  from (select t.operemp_chr, trunc(t.operdate_dat) as operdate_dat
        ...
        select a.printemp_chr as operemp_chr,
         ...
         /// <returns></returns>
        [AutoComplete]
        public long m_lngGetCheckedOutDataByCondition(System.Security.Principal.IPrincipal p_objPrincipal,int m_intStatDateType, string m_strCheckManID,string m_strBalanceDeptID, string m_strBeginTime,string m_strEndTime, string strRptId,ArrayList fysfcdeptidARR, out DataTable m_dtCheckOutData)
        {
         ...
         b.itemcatid_chr, b.tolfee_mny, e.groupid_chr, e.groupname_chr
    from t_opr_outpatientrecipeinv a,
         t_opr_outpatientrecipesumde b,
         t_bse_patientpaytype f,
...
//                    where a.INVOICENO_VCHR=b.INVOICENO_VCHR and a.BALANCEFLAG_INT=1 and a.SEQID_CHR=b.SEQID_CHR  and a.PAYTYPEID_CHR=c.PAYTYPEID_CHR(+) and a.BALANCE_DAT=To_Date('" + strDate + "','yyyy-mm-dd HH24:mi:ss') and BALANCEEMP_CHR='" + BALANCEEMP + "'  order by a.INVOICENO_VCHR,a.SEQID_CHR";
        ...
        [AutoComplete]
        public long GetCheckOutHistory(System.Security.Principal.IPrincipal p_objPrincipal, string strDate, string BALANCEEMP, string strRptId, out DataTable dtCheckOut)
        {
            if (strRptId == null || strRptId == "")
            {
                throw new Exception("报表的Id号为空，请从功能菜单传入报表Id号。");
