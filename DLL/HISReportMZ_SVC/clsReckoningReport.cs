...
namespace com.digitalwave.iCare.middletier.HIS.Reports
{
	...
	public class clsReckoningReport:com.digitalwave.iCare.middletier.clsMiddleTierBase
	{
		...
		[AutoComplete]
		public long m_lngFindByDateReport(System.Security.Principal.IPrincipal p_objPrincipal,int p_intSelectedIndex,string p_strName,string finddate,out System.Data.DataTable p_tabReport,out System.Data.DataTable p_tabReportdetial)
		{
		...
		/// <returns></returns>
		[AutoComplete]
		public long m_lngMeditionProtectReport(System.Security.Principal.IPrincipal p_objPrincipal,string finddate,string finddateLast,string type,out System.Data.DataTable p_tabReport)
		{
			string strSQL;
			p_tabReport=new DataTable();
...
//            " AND TO_DATE('" + finddateLast + " 23:59:59','yyyy-mm-dd hh24:mi:ss') order by t1.invdate_dat";
            //====2007-09-20 用到（门诊结算表）的新SQL语句
//            strSQL = @"select   t1.chargeno_chr, t1.operdate_dat, t1.recdate_dat,
//         t1.totalsum_mny as totalsum_mny, t2.idcard_chr as patientid_chr,
//         t2.lastname_vchr
//    from t_opr_charge t1, t_bse_patient t2,t_opr_chargedefinv t3,t_opr_outpatientrecipeinv t4
//   where t1.patientid_chr = t2.patientid_chr(+)
//     and t1.chargeno_chr = t3.chargeno_chr
//     and t3.invoiceno_vchr = t4.invoiceno_vchr
//     and t1.paytypeid_chr in (" + type + @")
//     and (t4.status_int = 1 or t1.status_int = 3)
//     and t1.recdate_dat is not null
//     and t1.operdate_dat BETWEEN TO_DATE('" + finddate + "','yyyy-mm-dd hh24:mi:ss') " +
//            " AND TO_DATE('" + finddateLast + " 23:59:59','yyyy-mm-dd hh24:mi:ss') order by t1.operdate_dat";

            strSQL = @"select   t1.chargeno_chr, t1.operdate_dat, t1.recdate_dat,
                             t1.totalsum_mny as totalsum_mny, t2.idcard_chr as patientid_chr,
                             t2.lastname_vchr
                        from t_opr_charge t1, t_bse_patient t2
                       where t1.patientid_chr = t2.patientid_chr(+)
                         and t1.paytypeid_chr in (" + type + @")
                         and t1.status_int = 1
                         and (t1.type_int = 1 or t1.type_int = 3)
                         and (t1.operdate_dat between ? and ?)
                    order by t1.operdate_dat";
			...
			{				
                ...
                ParamArr[0].Value = Convert.ToDateTime(finddate.Trim() + " 00:00:00");
                ParamArr[1].Value = Convert.ToDateTime(finddateLast.Trim() + " 23:59:59");

                lngRes = objHRPSvc.lngGetDataTableWithParameters(strSQL, ref p_tabReport, ParamArr);
				objHRPSvc.Dispose();	
		...
		/// <returns></returns>
		[AutoComplete]
		public long m_lngPublicPayReport(System.Security.Principal.IPrincipal p_objPrincipal,int fromt,string p_patientID,string finddate,string p_toDate,out System.Data.DataTable p_tabReport)
		{
			string strSQL;
			p_tabReport=new DataTable();
			...
			clsPrivilegeHandleService objPrivilege = new clsPrivilegeHandleService();
			//检查是否有使用些函数的权限
			lngRes = objPrivilege.m_lngCheckCallPrivilege(p_objPrincipal,"com.digitalwave.iCare.middletier.HIS.clsReckoningReport","m_lngPublicPayReport");
			if(lngRes < 0) //没有使用的权限
			{
				return -1;
			}
	...
	public class clsDifficultyReportOfMonthSvc:com.digitalwave.iCare.middletier.clsMiddleTierBase
	{
		...
		#region 获取数据
		[AutoComplete]
		public long m_mthGetAllDataOfMonth(System.Security.Principal.IPrincipal p_objPrincipal,DateTime date,DateTime date2,out System.Data.DataTable dt,out System.Data.DataTable dt1,out System.Data.DataTable dt2)
		{
