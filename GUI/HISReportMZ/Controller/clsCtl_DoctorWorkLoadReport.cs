...
//			}
            ...
            if (m_strFlag == "0")
            {
                this.m_objViewer.myPrintPreViewControl1.ReportName = this.strReportName+"(按发生时间)";
            }
            else

            {
                this.m_objViewer.myPrintPreViewControl1.ReportName = this.strReportName+"(按结帐时间)";
            }
		}
		#endregion
		#region 生成表结构
		private DataTable m_mthCreatTable()
		{
            ...
            if (this.m_objViewer.radioButton2.Checked)
            {
                ret.Columns.Add("医生工号", typeof(String));
            }
...
//			}
			if(this.m_objViewer.txtCode.Tag!=null&&this.m_objViewer.txtCode.Tag.ToString().Trim()!="")
			{
				...
				else
				{
					...
					obj.m_strOwnerName ="医生名称:"+this.m_objViewer.txtName.Text+"  医生工号:"+this.m_objViewer.txtCode.Text;
		...
		public void m_mthGetMultWorkLoadData(int flag,string m_strFlag)
		{
			...
			else
			{
				...
				for(int i2 =0;i2<this.m_objViewer.listView2.Items.Count;i2++)
				{
                    ...
                    dr["医生工号"] = this.m_objViewer.listView2.Items[i2].SubItems[0].Text;
