...
namespace com.digitalwave.iCare.gui.HIS.Reports
{
    ...
    public class clsCtl_GroupWorkLoadReport : com.digitalwave.GUI_Base.clsController_Base
    {
        ...
        public void m_mthCreatTable()
        {
            ...
            Mydt.Columns.Add("急诊", typeof(String));
            Mydt.Columns.Add("日诊", typeof(String));
            DataTable dtTemp;
        ...
        public void m_mthGetMultWorkLoadData(int flag)
        {
            ...
            if (l > 0 && objSubArr != null)
            {
                ...
                for (int i = 0; i < objSubArr.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        for (int intCount = 0; intCount < dtTempZFS.Rows.Count; intCount++)
                        {
                            ...
                            {
                                dr["正方"] = dtTempZFS.Rows[intCount]["正方"].ToString().Trim() == "0" ? "" : dtTempZFS.Rows[intCount]["正方"].ToString().Trim();
                                //dr["副方"] = dtTemp.Rows[intCount]["副方"].ToString().Trim() == "0" ? "" : dtTemp.Rows[intCount]["副方"].ToString().Trim();
                                drMain["正方"] = clsConvertToDecimal.m_mthConvertObjToDecimal(drMain["正方"]) + clsConvertToDecimal.m_mthConvertObjToDecimal(dtTempZFS.Rows[intCount]["正方"]);
                        ...
                        for (int intCount = 0; intCount < dtTempFFS.Rows.Count; intCount++)
                        {
                            if (strGroupTempName == dtTempFFS.Rows[intCount]["GROUPNAME_VCHR"].ToString().Trim())
                            {
                                ...
                                dr["副方"] = dtTempFFS.Rows[intCount]["副方"].ToString().Trim() == "0" ? "" : dtTempFFS.Rows[intCount]["副方"].ToString().Trim();
                               // drMain["正方"] = clsConvertToDecimal.m_mthConvertObjToDecimal(drMain["正方"]) + clsConvertToDecimal.m_mthConvertObjToDecimal(dtTemp.Rows[intCount]["正方"]);
                                drMain["副方"] = clsConvertToDecimal.m_mthConvertObjToDecimal(drMain["副方"]) + clsConvertToDecimal.m_mthConvertObjToDecimal(dtTempFFS.Rows[intCount]["副方"]);
                                break;
                            }
                        }
                        for (int intCount = 0; intCount < dtPersonNums.Rows.Count; intCount++)
                        {
                            if (strGroupTempName == dtPersonNums.Rows[intCount]["GROUPNAME_VCHR"].ToString().Trim())
                            {
                                ...
                                dr["急诊"] = dtPersonNums.Rows[intCount]["急诊"].ToString().Trim() == "0" ? "" : dtPersonNums.Rows[intCount]["急诊"].ToString().Trim();
                                drMain["急诊"] = clsConvertToDecimal.m_mthConvertObjToDecimal(drMain["急诊"]) + clsConvertToDecimal.m_mthConvertObjToDecimal(dtPersonNums.Rows[intCount]["急诊"]);

                                dr["日诊"] = dtPersonNums.Rows[intCount]["日诊"].ToString().Trim() == "0" ? "" : dtPersonNums.Rows[intCount]["日诊"].ToString().Trim();
                                drMain["日诊"] = clsConvertToDecimal.m_mthConvertObjToDecimal(drMain["日诊"]) + clsConvertToDecimal.m_mthConvertObjToDecimal(dtPersonNums.Rows[intCount]["日诊"]);
                                break;
                            }
                        }
                ...
                for (int intCount = 0; intCount < dtTempZFS.Rows.Count; intCount++)
                {
                    ...
                    {
                        dr["正方"] = dtTempZFS.Rows[intCount]["正方"].ToString().Trim() == "0" ? "" : dtTempZFS.Rows[intCount]["正方"].ToString().Trim();
                       // dr["副方"] = dtTempZFS.Rows[intCount]["副方"].ToString().Trim() == "0" ? "" : dtTempZFS.Rows[intCount]["副方"].ToString().Trim();
                        drMain["正方"] = clsConvertToDecimal.m_mthConvertObjToDecimal(drMain["正方"]) + clsConvertToDecimal.m_mthConvertObjToDecimal(dtTempZFS.Rows[intCount]["正方"]);
                       // drMain["副方"] = clsConvertToDecimal.m_mthConvertObjToDecimal(drMain["副方"]) + clsConvertToDecimal.m_mthConvertObjToDecimal(dtTemp.Rows[intCount]["副方"]);
                        break;
                    }
